$(function () {
    ...
    chrome.tabs.sendMessage(tabs[0].id, {
      ...
      if (response) {
        if (response.status === 'resumeDetail') {
          ...
          let resume = analysisResume(response.data, response.channel,response.subType);
          initForm(resume);
  ...
  ivvaImportResume.onMessage.addListener(function (msg) { //监听接收消息
    if (msg.status === 'validateResume') {
      ...
      if (msg.data.success) {
        ...
        if (msg.data.listData.length === 0) {
          text = '系统中未发现重复或疑似的简历'
        } else {
          text = `系统中发现<strong style="color: red">${msg.data.listData.length}</strong>份重复或疑似的简历`;
          let resumeList = "";
          for(let item of msg.data.listData){
            resumeList+=`<p class="similar-item-name-wrap"><a class="similar-item" href="#" data-resume-id=${item.resumeId}>${item.name}--的简历</a><a class="similar-item" data-resume-id=${item.resumeId} href="#">点击查看</a></p>`
          }
  ...
  function initForm(resume) {
    let form = `<div class="form" data-id=${resume.id}>
        ...
        <div class="form-item">
          <div class="ivva-flexAlignCen">
            <input type="checkbox" class="checkbox" id="talent"> <label for="talent">放入人才库</label>
          </div>
        </div>
        <div class="form-item">
          <button id="importBtn" type="button">导入简历</button>
        </div>
      </div>`
  ...
  function importSuccess(data) {
    let html = `<div class="success-page">
      ...
      <div class="success-btn">
        <button type="button" id="detail-btn">打开候选人详情</button>
        <!--<button type="button" id="recom-btn">推荐给用人部门</button>-->
      </div>
    </div>`;
...
//解析简历
  function analysisResume(domHtml, channel,type) {
    let resume = {};
    ...
    if (channel === "zl") {
      if(type===2){
        ...
        resume.school = $(domHtml).find(".resume-content__section span[data-bind='textQ: data.schoolName']").text();
      ...
      let name = $(domHtml).find('#tdseekname').text();
      if (name.indexOf('ID') == -1) {
        resume.name = $.trim(name && name.split('流程')[0])
        resume.name = $.trim(name && name.split('和Ta聊聊')[0])
      } else {
        resume.name = ""
      };
      let info = "";
      ...
      resume.id="";
      if(type == 1){
        resume.name = $(domHtml).find('.mr_name').text();
        resume.sex = $(domHtml).find('.base_info .s').text()==='男'?1:2;
        resume.jobYear = $(domHtml).find('.base_info .job_span').text();
        resume.city = $(domHtml).find('.base_info .d').text();
        resume.education = $(domHtml).find('.base_info .x').text();
        let company = $(domHtml).find('.shenfen em').text();
        if(company){
          resume.company = company.split('·')[1]
        }
        resume.school = $(domHtml).find('#educationalBackground .mr_jobe_list:first-child h4').text()
        resume.mobile = "";
        resume.email = "";
        resume.id = $(domHtml).find('.user-tollbar-chat .recommend-chat-chat:first-child ').data('resumekey')
      }else if(type==2){
